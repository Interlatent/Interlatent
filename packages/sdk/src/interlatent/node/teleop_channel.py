...
@dataclass
class TeleopFrame:
    """Decoded WS frame from the dashboard.

    ``engaged`` decides whether the control loop overrides the policy.
    ``held_keys`` is the set of currently-held key strings (case-
    insensitive). ``deadman``, when False, forces engaged=False.
    """
    engaged: bool
    deadman: bool
    held_keys: set[str]
    ...
    def from_json(cls, raw: str) -> "Optional[TeleopFrame]":
        ...
        return cls(
            ...
            held_keys={
                str(k).lower() for k in (obj.get("held_keys") or [])
                if isinstance(k, str)
            },

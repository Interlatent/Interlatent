...
Two usage modes:
    ...
    Mode 1 – standalone collection (drives its own rollout loop):
        ...
        client.watch(runner.alg.actor, env, environment="spot-velocity", layer="auto")
...
class IsaacSimCollectionEnv(RslRlVecEnvWrapper):
    ...
    def inspect_rewards(self, env_idx: int = 0) -> dict[str, dict[str, Any]]:
        ...
        Returns an empty dict for DirectRLEnv (no reward manager).
        """
    ...
    def reward_config_json(self, env_idx: int = 0) -> dict[str, Any] | None:
        """Serialize the current RewardManager state as a JSON-ready dict.

        Returns ``None`` for DirectRLEnv (no reward manager).
        """
    ...
    def _build_action_labels(self) -> list[str]:
        ...
        Returns an empty list for DirectRLEnv (no action manager).
        """
    ...
    def _make_obs_context_fn(
        ...
        Returns ``None`` when neither obs nor action labels could be derived
        (e.g. DirectRLEnv or unknown group), in which case the SDK falls back
        to ``obs_0`` / ``obs_1`` naming.
        """

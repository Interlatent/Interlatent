...
class PolicyRuntime:
    ...
    def load(
        ...
        **backend_kwargs: Any,
        ...
        # Only forward chunk_size / action_dim when the caller actually set them
        # (non-zero). 0 means "unspecified" — let the backend choose: cfg-bearing
        # policy backends (lerobot / molmoact2) resolve the model's NATIVE
        # chunk_size + action_dim from its config, while toy backends
        # (echo / tiny_torch / mlp) fall back to their own constructor defaults.
        # This is what stops the pre-warm (which omits both) from baking a 32/6
        # default into the cached runtime and shadowing the model's real 100/16
        # shape on every later OpenSession that hits the cache.
        shape_kwargs: dict[str, Any] = {}
        if chunk_size:
            shape_kwargs["chunk_size"] = chunk_size
        if action_dim:
            shape_kwargs["action_dim"] = action_dim
        impl = factory(**shape_kwargs, **backend_kwargs)

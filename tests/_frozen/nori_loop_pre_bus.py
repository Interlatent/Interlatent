"""``adapters/nori/loop.py::control_loop`` frozen before the runner+bus migration.

A copy of the native Nori loop as of commit ``74c638b`` (PR 4 — the last
commit where the inline while-loop was the live implementation), including
the PR 1 safety hotfix (level-triggered e-stop rung, policy-path delta
...
rewritten absolute. They stay *inside* the function, exactly as in the
original, so monkeypatches installed on those modules before the call are
picked up identically by the frozen and the live loop. Do not "fix"
anything here; a frozen reference that drifts stops being a reference.
...
def control_loop(
    ...
    **_: Any,
    ...
    from interlatent.adapters.nori.config import build_adapter_config
